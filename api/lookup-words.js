...
export default async function handler(request,response) {
  ...
  try {
    ...
    const data=await structuredCompletion({
      ...
      user:`For every requested word, give its specific Japanese meaning in this passage, concrete basic dictionary meaning(s) in Japanese, and English part of speech. Never answer only “文脈に依存” or another vague placeholder.\n\nWORDS: ${requested.join(', ')}\n\nPASSAGE:\n${String(passage).slice(0,14000)}`,
      schema,
    });
