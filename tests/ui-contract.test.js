...
test('summary grading uses six elements plus a five-point impression and code-side display cap',()=>{ assert.match(gradeApi,/minItems:6,maxItems:6/); assert.match(gradeApi,/impressionScore/); assert.match(gradeApi,/A list of remembered details is not a good summary/); assert.match(app,/normalizeSummaryGrade/); assert.match(app,/summaryElementScore/); assert.match(app,/summaryImpressionScore/); assert.match(app,/rawSummaryScore/); });
