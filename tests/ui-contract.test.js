...
test('Long has WPM50 reading limit, concrete vocabulary, and structured summary feedback',()=>{
  assert.match(app,/longReadingLimitSeconds/); assert.match(app,/resolveUnknownWords/); assert.match(app,/contextMeaning/); assert.match(gradeApi,/goodPoints/); assert.match(gradeApi,/missingPoints/); assert.match(gradeApi,/modelSummary/);
});
...
test('version 2.0 is visible and package version is aligned',()=>{ assert.match(html,/ver 2\.0/); assert.match(app,/VER 2\.0/); assert.equal(packageJson.version,'2.0.0'); });
test('summary grading uses six elements, floors the display score, and adds only that score to the Long total',()=>{ assert.match(gradeApi,/minItems:6,maxItems:6/); assert.match(gradeApi,/impressionScore/); assert.match(gradeApi,/A list of remembered details is not a good summary/); assert.match(app,/normalizeSummaryGrade/); assert.match(app,/summaryElementScore/); assert.match(app,/summaryImpressionScore/); assert.match(app,/rawSummaryScore/); assert.match(app,/summaryScore=grade\.displayScore; const multipleChoiceScore=mcCorrect\*3; const totalScore=multipleChoiceScore\+summaryScore/); });
