...
test('question timer is a viewport-fixed sibling outside scrollable question content',()=>{ assert.match(app,/mode==='quick'\?10:15/); assert.match(app,/app\.innerHTML=`<div class="question-toolbar fixed-question-timer"[\s\S]*<section class="focus-question">/); assert.match(styles,/\.question-toolbar \{ position:fixed; top:0; left:0; right:0; z-index:9999/); assert.doesNotMatch(styles,/\.question-toolbar \{[^}]*position:(?:-webkit-)?sticky/); assert.match(styles,/body\.question-active \.focus-question \{ padding-top:calc\(66px \+ env\(safe-area-inset-top\)\)/); assert.match(styles,/min-height:calc\(50px \+ env\(safe-area-inset-top\)\)/); });
...
test('version 1.2 is visible and package version is aligned',()=>{ assert.match(html,/ver 1\.2/); assert.equal(packageJson.version,'1.2.0'); });
