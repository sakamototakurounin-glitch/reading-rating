...
export const updateRatingWithCalibration = (oldRating, actualAccuracy, expected, completedLongSessions) => {
  const base = updateRating(oldRating, actualAccuracy, expected);
  const multiplier = completedLongSessions < RATING.calibrationSessions ? RATING.calibrationMultiplier : 1;
  const delta = base.delta * multiplier;
  return { delta, multiplier, newRating: Math.max(0, oldRating + delta) };
};
