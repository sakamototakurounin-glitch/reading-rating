...
export const displaySummaryScore=(rawScore)=>Math.min(10,Math.floor(clamp(Number(rawScore)||0,0,11)));
...
export function normalizeSummaryGrade(grade) {
  ...
  return {...grade,elements,elementScore,impressionScore,rawScore,displayScore:displaySummaryScore(rawScore),feedback:String(grade.feedback||''),modelSummary:String(grade.modelSummary||grade.modelAnswer||'')};
}
