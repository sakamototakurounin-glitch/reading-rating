...
export function buildDifficultyProfile(value) {
  ...
  return Object.freeze({
    ...
    syntaxComplexity: smooth(0.06, 0.94, n),
    ...
    targetSentenceWords: Math.round(11 + 13 * n),
...
export function difficultyPrompt(profile) {
  ...
  return `Use this continuous generation profile: ${JSON.stringify(p)}. ${low} ${middle} ${high} Keep each step from 0 to 17 perceptibly but smoothly harder. Rare vocabulary is tightly capped; even at 17, logic, abstraction, syntax, discourse, and inference must carry most of the difficulty. Aim for an average sentence length near ${p.targetSentenceWords} words while varying sentences naturally.`;
}
